    ...
    <>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {service.tools.map((tool) => (
                  <span key={tool} className="font-mono text-[11px] tracking-wider text-accent-dim px-3 py-1.5 border border-accent/12 rounded-full">
                    {tool}
                  </span>
                ))}
              </div>
      ...
      <Section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-px bg-border border border-border rounded-2xl overflow-hidden">
          {process.map((item) => (
            <div key={item.step} className="bg-bg p-10 transition-all duration-400 hover:bg-bg-card group">
              <div className="font-display text-[56px] text-border-light leading-none mb-5 transition-colors duration-400 group-hover:text-accent">
                {item.step}
              </div>
